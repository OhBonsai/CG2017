...
Bonsai.Primatives = {
    Quad: function () {
        if (Bonsai.Res.Vao["BonsaiQuad"]) return Bonsai.Res.Vao["BonsaiQuad"];

        let aVert = [-0.5, 0.5, 0, -0.5, -0.5, 0, 0.5, -0.5, 0, 0.5, 0.5, 0],
            aUV = [0, 0, 0, 1, 1, 1, 1, 0],
            aIndex = [0, 1, 2, 2, 3, 0];
        return Bonsai.Shaders.VAO.standardMesh("BonsaiQuad", 3, aVert, null, aUV, aIndex, false);
    },
    ...
    FacedCube: function (keepRawData) {
        ...
        return Bonsai.Shaders.VAO.standardMesh("BonsaiFCube", 4, aVert, aNorm, aUV, aIndex, false);
    }
};